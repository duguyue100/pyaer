...
def drawing_func(in_q):
    while threading.currentThread().isAlive():
        try:
            (pol_events, num_pol_event, special_events, num_special_event) = in_q.get()
            if num_pol_event != 0:
...
if __name__ == "__main__":
    # define thread
    q = Queue(maxsize=1)
    drawer = threading.Thread(name="drawer", target=drawing_func, args=(q,))
    fetcher = threading.Thread(name="fetcher", target=fetching_func, args=(q,))
